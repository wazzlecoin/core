        ...
        for (auto m : setMints) {
            ...
            if (m.hashStake == 0) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->zibtcTracker->UpdateState(m);
                }
            }
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZibtcSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, ispublicspend);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZibtcSpend(nAmount, false, true, vMintsSelected, address_str);
}


extern UniValue DoZibtcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
{
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzBXCTracker* zibtcTracker = pwalletMain->zibtcTracker.get();
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
...

    {
        int nTries = 0;
        while (nTries < 100) {
            TRY_LOCK(zibtcTracker->cs_spendcache, fLocked);
            if (fLocked) {
                if (zibtcTracker->ClearSpendCache()) {
