...
#define URI_SCHEME "bitcoinclassic"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinClassic address (e.g. %1)").arg("X7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no BitcoinClassic: URI
...
{
    // Convert bitcoinclassic:// to bitcoinclassic:
    //
    //    Cannot handle this later, because bitcoinclassic:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open bitcoinclassic.conf with the associated application */
...
{
    return GetAutostartDir() / "bitcoinclassic.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bitcoinclassic.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bitcoinclassic app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bitcoinclassic app to startup item list
