...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zibtcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zibtcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zIBTC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zIBTC seed used to mint this zIBTC is not the same as currently hold in the wallet";
            else
