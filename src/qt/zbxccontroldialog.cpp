...
#include "ui_zbxccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbxcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbxcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBXC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBXC seed used to mint this zBXC is not the same as currently hold in the wallet";
            else
